...
if TRAINING: ## Training should be set to True above if we want to train
    ...
    for i in range(0, EPOCHS): ## Loops through epochs in range up to the epoch number specified earlier
        ...
        if best_iou_score < valid_logs['iou_score']: ## Determines if the new IoU is better than the previous
            ...
            torch.save(model, f'./ResNet50_n5_e30_Focal.pth') ## Save the new model
            print('Model saved!')
...
train_logs_df.to_csv(r'./RN50_train_n5_e30.csv', sep='\t', encoding='utf-8', header='true') ## Saves dataframes as CSVs
valid_logs_df.to_csv(r'./RN50_val_n5_e30.csv', sep='\t', encoding='utf-8', header='true')
...
plt.title('ResNet50 IoU Score over Epoch', fontsize=21)
plt.legend(loc='best', fontsize=16)
plt.grid()
plt.savefig('ResNet50_n5_e30_IoU.png')
plt.show()
...
plt.title('ResNet50 Dice Loss Plot over Epoch', fontsize=21)
plt.legend(loc='best', fontsize=16)
plt.grid()
plt.savefig('ResNet50_n5_e30_Dice.png')
plt.show()
